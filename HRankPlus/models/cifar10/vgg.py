...
from models.cifar10.quan_vgg_cifar import quan_Conv2d, quan_Linear  # Import quantization layers
...
class VGG(nn.Module):
    def __init__(self, compress_rate, cfg=None, num_classes=10):
        ...
        self.classifier = nn.Sequential(OrderedDict([
            ('linear1', quan_Linear(cfg[-2], cfg[-1])),  # Use quan_Linear
            ('norm1', nn.BatchNorm1d(cfg[-1])),
            ('relu1', nn.ReLU(inplace=True)),
            ('linear2', quan_Linear(cfg[-1], num_classes)),  # Use quan_Linear
        ]))
    ...
    def _make_layers(self, cfg):
        ...
        for i, x in enumerate(cfg):
            if x == 'M':
                ...
                cnt+=1
                conv2d = quan_Conv2d(  # Use quan_Conv2d
